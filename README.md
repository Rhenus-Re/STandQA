...
`module2-ai-testing` 已汇总 A、B、C 三个测试域的模块二成果。AI 测试共 37 条独立编号用例（参数化后为 47 个 pytest 项）：A 域 21 条、B 域 8 条、C 域 8 条。与模块一 42 个 pytest 项共同执行时，当前完整回归为 89 项全部通过。

| 测试域 | 主要材料 | 已确认的模块二缺陷 |
|---|---|---|
| A：编码与纠错 | 21 条 AI 用例、候选审核记录、基线、测试报告、缺陷清单、对比分析与 AI 反思 | M2-A-01：非正分段阈值导致编码分段无法结束 |
| B：对象与矩阵 | 8 条 AI 用例、审核记录、用例清单、缺陷清单、测试报告与成果汇报 | B-03：`error_correction` 非法值未被友好校验 |
| C：输出与命令行 | 8 条 AI 用例（参数化后 16 项）、候选审核记录、基线、测试报告、缺陷清单、对比分析与 AI 反思 | M2-C-01：CLI `--factory` 非法路径未友好处理 |

完整材料索引见 `reports/module2/README.md`，AI 提示词和审核记录见 `ai_records/`。模块二缺陷仍须由非发现者填写实际交叉复现记录后，才能作为最终课程证据。
