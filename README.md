...
修复 A-01、A-02、A-03、B-04 与 B-05 后，A 域测试为 13 个 pytest 项通过（对应 `TC-A-01` 至 `TC-A-12`，其中 TC-A-09 参数化 2 项），B 域测试为 17 项通过，合并后为 30 项通过。完整覆盖率结果为总覆盖率 51%、`base.py` 98%、`util.py` 96%、`main.py` 70%；`coverage.xml` 为可再生输出，不提交至仓库。
