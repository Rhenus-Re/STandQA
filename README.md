...
> 已引入 `python-qrcode v8.2` 被测源码。`member-c/output-commandline` 分支包含成员 C 的输出与命令行测试、C-01 修复和对应证据材料；`main` 历史不在该分支上改写。
...
| C-01 回归与修复 | `qrcode/main.py`（print_tty 静区） | 已完成本地回归；待成员 B 实际交叉复现 |
